...
const FoodQuantityTag = () => {
  const [sectionLabelSpotList, setSectionLabelSpotList] = useState({});
  const [randomSubSection2, setRandomSubSection2] = useState('');
  ...
  useEffect(() => {
    const fetchRandomSubSection2 = async () => {
      try {
        ...
        console.log('check randomMainSection2', randomSubSection2);

        //subSection1 값을 사용해 데이터 조회
...
export default FoodQuantityTag;
